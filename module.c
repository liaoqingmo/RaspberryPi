 ...
 * @date    23 Oct 2019
 * @version 0.1
 * @brief  LKM_Templete is a basic templete for linux LKM tty driver .
 * @see http://www.github.com/amaali7/LKM_Templete for a full description and follow-up descriptions.
*/
...
MODULE_AUTHOR("Abdallah Adam");      ///< The author -- visible when you use modinfo
MODULE_DESCRIPTION("A simple Linux driver for the Raspberry Pi tty driver.");  ///< The description -- see modinfo
MODULE_VERSION("0.1");              ///< The version of the module
 
static int gpio_list[] = {1,2,3,4};  

#define SOFT_UART_MAJOR            0
#define NONE                       0
#define TX_BUFFER_FLUSH_TIMEOUT 4000  // milliseconds
#define ARRAYS(array)	sizeof(array)/sizeof(array[0])
#define N_PORTS                    ARRAYS(gpio_list)/2
#define GPIO_CONTER                ARRAYS(gpio_list)
int COUNER = 0;

static int uart_init(void);
static int register_uart_to_minor(void);
...
static int uart_init(void){
	  ...
	  //tty_port_link_device(&port, soft_uart_driver, 1);
	#endif
...
static int register_uart_to_minor(void){
	int i=0;
	for (i; i < N_PORTS; ++i){
		if (!raspberry_soft_uart_init(gpio_list[i], gpio_list[i+1]))
		{
